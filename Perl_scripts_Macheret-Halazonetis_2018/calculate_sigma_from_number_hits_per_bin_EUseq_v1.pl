...
#
# 
#  script: calculate_sigma_from_number_hits_per_bin_EUseq_v1.pl
#  
...
#
#
#  usage: calculate_sigma_from_number_hits_per_bin_EUseq_v1.pl    EUseq_file_chr1-X_adjbin_EU_0b.csv
#
#  requires in same directory: "adjbin_0b.csv" and matching "adjbin_qual_counts_0b.txt" files
#      generated by script: calculate_number_hits_per_bin_EUseq_v1.pl
#
#
#
#
...
#
#
# FILE: -------_sigma_qual_counts_EU_0b.txt
#
...
#
$file_Name=$ARGV[0];
@temp_file_name1=split("adjbin_EU_0b.csv",$file_Name);
$quality_file_Name=join("",$temp_file_name1[0],"adjbin_qual_counts_EU_0b.txt");
#
if ($#ARGV!=0) {
    print "\nusage: calculate_sigma_from_number_hits_per_bin_EUseq_v1.pl    EUseq_file_chr1-X_adjbin_EU_0b.csv\n";
...
if (!-r "$file_Name" ) {
    print "\nusage: calculate_sigma_from_number_hits_per_bin_EUseq_v1.pl    EUseq_file_chr1-X_adjbin_EU_0b.csv\n";
    print "file type:  EUseq_file_chr1-X_adjbin_EU_0b.csv   non-existent or not readable\n\n";
    exit;
}
#
if (!-r "$quality_file_Name" ) {
    print "\nusage: calculate_sigma_from_number_hits_per_bin_EUseq_v1.pl    EUseq_file_chr1-X_adjbin_EU_0b.csv\n";
    print "program requires file type:    EUseq_file_chr1-X_adjbin_qual_counts_EU_0b.txt    in the same directory\n";
...
for ($loopC=1; $loopC<=23; $loopC++) {
    for ($loopA=0; $loopA<$chr_size[$loopC]; $loopA++){
        if ($adjust[$loopC][$loopA]!=0) {  #recommended for high quality EUseq data with no reads in intergenic regions
        #if ($adjust[$loopC][$loopA]!=0 && $adjbin[$loopC][$loopA]>0) {  #not recommended for high quality EUseq data with no reads in intergenic regions
...
#
#
if ($percentile_baseline_cutoff_high==$percentile_baseline_cutoff_low) {
    $percentile_baseline_cutoff_high=$percentile_baseline_cutoff_low+20;
    if ($percentile_baseline_cutoff_high>89) {$percentile_baseline_cutoff_high=$percentile_baseline_cutoff_low+1} 
    $adjbin_baseline_cutoff_high=$adjbin_baseline_cutoff_low*1.1;
}
...
for ($loopC=$percentile_baseline_cutoff_low; $loopC<=$percentile_baseline_cutoff_high; $loopC++) {  # $percentile_baseline_cutoff_low   is equal to  $loopC-11  above 
    ...
    if ( abs($diff_median_mean)<=$min_diff_mean_median && $adjbin_baseline_SD>0) {
...
print "Number of adjbin data values used to calculate mean and SD: $nAdjBin\n\n";
#
if ($adjbin_baseline_mean==0 || $adjbin_baseline_SD==0 || $nAdjBin==0) {
    $flag_power_curve=0;
    print "No Background Noise in Sample\nUsing expected power curve, instead of actual curve\nOutput values are relative sigma\n\n";
} else {
    $flag_power_curve=1;
}
#
...
#
if ($flag_power_curve==1) {          ######################## $flag_power_curve==1
#
#
@non0_adjust_noise=();
for ($loopC=1; $loopC<=23; $loopC++) {
...
if ($number_points_line<10) {
    print "Script Line 614: Number_points_line = $number_points_line too small. Increase number_split_adj_read in Line 517\n\n";
    exit;
}
#
...
#
#
} ################ end  $flag_power_curve==1
#
if ($flag_power_curve==0) {          ######################## $flag_power_curve==0
#
#
@non0_adjust_noise=();
for ($loopC=1; $loopC<=23; $loopC++) {
...
#
 ...
 $SLOPE_pow=-0.2;
 $INTER_pow=6;
 print "Estimated Power curve stats: (adjust reads start position: $start_power_curve  end position: $end_power_curve)\n";
 print "SLOPE: $SLOPE_pow  ";
 #print "SD OF SLOPE: $SDSLOPE\n";
 print "INTERCEPT: $INTER_pow  ";
...
for ($loopE=1; $loopE<=$number_split_adj_read; $loopE++) {
    ...
    $calc_adjbin_baseline_SD_split_adj_read=exp($log_adjbin_baseline_SD_split_adj_read);
    if ( $loopE<10 || $loopE==(int(($loopE)/10))*10 ) {
        print "$loopE\t\t$adjust_limit[$loopE]\t\t0\t\t$calc_adjbin_baseline_SD_split_adj_read\n";
    }
}
print "\n\n";
#
#
}  ################ end  $flag_power_curve==0
#
#
######################### 
#
...
$result_File = join("",$temp_file_name1[0],"sigma_all_EU_0b.csv");
#
...
#
#
$result_File = join("",$temp_file_name1[0],"sigma_select_EU_0b.csv");
#
open RESULT, "> $result_File";
#
for ($loopA=0; $loopA<$max_number_bins; $loopA++) {
...
#
#
$result_File = join("",$temp_file_name1[0],"sigma_qual_counts_EU_0b.txt");
#print "$result_File\n";
#
open RESULT, "> $result_File";
#
...
#
printf RESULT "\nNumber of bins interpolated:            $counts_interpolation\n";
