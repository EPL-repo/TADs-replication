...
#
# 
#  script: calculate_number_hits_per_bin_v1.pl
#  
...
#
#
#  usage: calculate_number_hits_per_bin_v1.pl    bin_size   quality   read_length   sam_file   
#         calculate_number_hits_per_bin_v1.pl    10000      60        100           repl_file.sam
#
#
#
print "\n";
...
if ($bin_size ne $temp_bin_size_2[0]) {
    print "\n\nusage:  calculate_number_hits_per_bin_v1.pl    bin_size   quality   read_length   sam_file\n";
    print "usage:  calculate_number_hits_per_bin_v1.pl    10000      60        100           repl_file.sam\n";
...
while ( <FILE> ) {
    ...
    if ($#line>10) {
        ...
        if ($line[4]>=$quality_score && $line[5] eq $read_length && $chr_number<24 && $chromosome!~/mchr/) {  #some sam files may contain mouse sequences for calibration
             ...
             $bin_hits[$chr_number][$bin_allocate_int] ++;
        } else {
             $qualnotOK ++;
        }
    }
...
for ($loopc=1; $loopc<=23; $loopc++){
   for ($loopb=0; $loopb<=$number_bins; $loopb++){
       $bin_total_hits=$bin_total_hits+$bin_hits[$loopc][$loopb];
...
for ($loopc=1; $loopc<=23; $loopc++){
   for ($loopb=0; $loopb<=$number_bins; $loopb++){
       if ($adjust_hits[$loopc][$loopb]>0) {     
           $adjbin_hits[$loopc][$loopb]=int($bin_hits[$loopc][$loopb]*$Acorr_factor*1000/$adjust_hits[$loopc][$loopb]+0.5);
           $adjbin_total_hits=$adjbin_total_hits+$adjbin_hits[$loopc][$loopb];
       } else {
           $adjbin_hits[$loopc][$loopb]=0;
       }
   }
}
print "Total adjusted sample hits: $adjbin_total_hits\n\n";
#
#
# write files
#
$result_File = join("_",$file_name[0],"bin-size",$bin_size,"quality",$quality_score,"chr1-X_adjbin_qual_counts_0b.txt");
#
open RESULT, "> $result_File";
#
printf RESULT "\n";
printf RESULT "file $sam_repl_File\n";
printf RESULT "quality OK $qualOK\n";
...
printf RESULT "Total adjusted sample hits: $adjbin_total_hits\n\n";
#
close RESULT;
#
#
#
#
$result_File = join("_",$file_name[0],"bin-size",$bin_size,"quality",$quality_score,"chr1-X_adjbin_0b.csv");
#
open RESULT, "> $result_File";
#
for ($loopb=0; $loopb<=$number_bins; $loopb++){
   for ($loopc=1; $loopc<=23; $loopc++){
       printf RESULT "$bin_hits[$loopc][$loopb],$adjbin_hits[$loopc][$loopb],$adjust_hits[$loopc][$loopb],,";
   }
   printf RESULT "\n";
}
#
